...
def convert_to_index(emotion):
    ...
    map_emo = {'anger':0, 'joy':1, 'neutral':2, 'sadness':3}
...
def split_dialog(dialogs):
  ...
  for dialog_id in dialogs.keys():
    # check who is in this dialog by going through this dialog
    spk_set = set()
    for utt in dialogs[dialog_id]:
      spk_set.add(utt.split('_')[-1])
    
    for spk in spk_set:
      spk_dialogs[dialog_id+'_'+spk] = []
    
    for utt in dialogs[dialog_id]:
      spk_dialogs[dialog_id+'_'+utt.split('_')[-1]].append(utt)
