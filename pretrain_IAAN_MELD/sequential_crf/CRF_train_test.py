...
if __name__ == "__main__":
    ...
    val_data = []
    test_data = []
    for dialog in train_dias:
        train_data.append((train_dias[dialog], []))
        for utt in train_data[-1][0]:
            train_data[-1][1].append(train_emo_dict[utt])
    for dialog in val_dias:
        val_data.append((val_dias[dialog], []))
        for utt in val_data[-1][0]:
            val_data[-1][1].append(val_emo_dict[utt])
    for dialog in test_dias:
        test_data.append((test_dias[dialog], []))
