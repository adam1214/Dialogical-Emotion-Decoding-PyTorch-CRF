...
if __name__ == "__main__":
    ...
    parser.add_argument('-v', "--pretrain_version", type=str, help="which version of pretrain model you want to use?", default='type_6')
    ...
    out_dict = joblib.load('../data/'+ args.pretrain_version + '/outputs_6_all_fold.pkl')
    dialogs = joblib.load('../data/dialog_iemocap.pkl')
    dialogs_edit = joblib.load('../data/dialog_6emo_iemocap.pkl')
    
    if args.dataset == 'original':
        emo_dict = joblib.load('../data/emo_all_iemocap_6.pkl')
        dias = dialogs_edit
    elif args.dataset == 'U2U':
        emo_dict = joblib.load('../data/'+ args.pretrain_version + '/U2U_6emo_all_iemocap_all_fold.pkl')
        dias = dialogs
    ...
    for i in range(0, len(label_val), 1):
        if label_val[i] == 'exc':
            ...
            label_val[i] = 1
        elif label_val[i] == 'fru':
            label_val[i] = 2
        elif label_val[i] == 'sad':
            label_val[i] = 3
        elif label_val[i] == 'hap':
            label_val[i] = 4
        elif label_val[i] == 'ang':
            label_val[i] = 5
        else:
            label_val[i] = -1
    
    model = CRF(len(utt_to_ix), emo_to_ix)
    optimizer = optim.SGD(model.parameters(), lr=0.01, weight_decay=2, momentum=0.5)
    ...
    for epoch in range(10):
        ...
        uar_val, acc_val, f1, conf_val = utils.evaluate(predict_val, label_val)
